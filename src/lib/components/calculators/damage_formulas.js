...
export function outgoing_dmg_2(scaling_attribute=0, cr = 5, cd = 50, skill_multiplier=0, 
                 dmg_bonus_p=0, def_ignore_p=0, res_shred=0,
                 vulnerability=0, extra_multiplier=0, extra_dmg=0, broken = false) {
    let final_dmg = (skill_multiplier / 100 + extra_multiplier) * scaling_attribute + extra_dmg;
    final_dmg *= 1 + dmg_bonus_p / 100;    
    let defense = 1000 * (1 - def_ignore_p / 100);
    final_dmg *= 1 - (defense / (defense + 200 + 10 * 80));   
